...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 relativeMouse =  iMouse.xy;
    
	vec2 dv = 1.*vec2(relativeMouse.x - uv.x, relativeMouse.y - uv.y);	
    ...
    dispacement *= slope*distortionAmount*tension;    
   // dispacement *= .1*vec2(SinWave(uv).r);
    //dispacement *=  vec2((.5+dv).x*.1,0.);
    
    //pul corners
    vec2 center = vec2(.5)*scale;
    vec2 moveToMouse = dv*vec2(-1,-1)*scale/30.; //<----cool
    float border = 0.1;
    // move to mouse
  ...
  //  col.rgb += pow(SinWave(pos).r,2.);
    ...
    vec3 layer = texture(uSampler,pos).rgb;
    layer += col;

    if(pos.x < 0. || pos.y < 0. || pos.x > (scale.x - 0.001) || pos.y > (scale.y - 0.001)  ){
        layer.rgb = vec3(1.,0.,1.);
        discard;
    }
	fragColor = vec4(layer,1.);
}
