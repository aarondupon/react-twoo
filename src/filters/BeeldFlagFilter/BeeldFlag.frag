...
// vertex dispacement
//https://www.shadertoy.com/view/4tlcz8

//https://codepen.io/shshaw/pen/qqPwXM
...
void CamPolar( out vec3 pos, out vec3 ray, in vec3 origin, in vec2 rotation, in float distance, in float zoom )
{
	...
	// ray in view space
	ray.xy = view_FragCoord.xy- view_Resolution.xy*.5;//(view_FragCoord.xy - view_Resolution.xy*.5);
...
{
	return texture(uSampler,uv).rgb;
// 	if ( view_Index == 0 )
// 	{
// 		// nyan cat
...
vec4 Weave( vec2 uv )
{
	vec2 a = uv*vec2(3.0,2.0)*dimensions.y*.85;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	view_Resolution = iResolution.xy;//vec2(280,1842);//iResolution.xy;
	// shadertoy do noting texture is square
	// view_FragCoord = fragCoord.xy ;
	
	// PIXI Inner sprite texture as base
	// squarise texture!
	float aspect = iResolution.x / iResolution.y;
	view_FragCoord = vec2(fragCoord.x/aspect,fragCoord.y);
	

	view_Index = 3;
	view_selectionRelativeMouse = vec4(1.,0.,1.,0.);//vec4(iMouse.x,iMouse.y,iMouse.x,iMouse.y)/view_Resolution.xyxy;

	//if ( SideMenu( ivec2(1,4) ) )
	//{
		vec3 pos, ray;
        vec3 origin = vec3(0.,0.,0.);
        vec2 rotation =  vec2(0,0);//vec2(-.8,-.5)+vec2(.9,1.5)*view_selectionRelativeMouse.yx;
		//CamPolar( pos, ray, origin, rotation, 10.0, 3.5 );
		//CamPolar( pos, ray, origin, rotation, 10., 3.5 );
		CamPolar( pos, ray, vec3(1.5,0,0), vec2(0.,0.),10.0, 3.5 );
		// if(view_FragCoord.x < 0.|| view_FragCoord.x > iResolution.x
		// 	|| view_FragCoord.y < 0.|| view_FragCoord.y > iResolution.y
		// ){
		// 	discard;
		// }
		float t = 0.0;// total distance
		float h = 1.0;// At this distance we are close enough to the object that we have essentially hit it
		for ( int i=-0; i < 17; i++ )
		{
			if ( h < .01 )
				break;
		...
		// vec3 col = texture(uSampler, pos.xy+vec2(0,.5)).rgb;
		// fragColor = vec4(col,1.0);
		vec2 uv = UVMapping( pos.xy );
		vec3 col = texture(uSampler, uv).rgb;
		...
		// col = mix( col, vec3(1,1,1)*(Noise(ray.xy*4.0).x*.3+1.0), weave.a );
