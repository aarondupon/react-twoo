...
const FluidShader = (props) => {
  ...
  var childrenWithProps = React.Children.map(props.children,
    ...
    //console.log('child::', child)
      return React.cloneElement(child, {
        ...
        onLoaded:(src,node,obj)=>{
          ...
          // pixi v3 var m =  renderer.currentRenderTarget.projectionMatrix.toArray(true)
          // pixi v4
        //  var m =  renderer.currentRenderer.projectionMatrix.toArray(true)

          var WIDTH = window.innerWidth/2;



          // create BUFFER! texture for position;



          //fillPositionTexture(gl,positionTexture,WIDTH,WIDTH)
          // create shader with texture of position  uniform positon in out position vertex;
          // var positionShader = createPasThruShader(gl,renderer,passThruUniforms)
           // add shader to pixi sprite and render in stage;
          //  var positionSprite = new PIXI.Sprite();
          //  positionSprite.shader = positionShader;
           //renderer.bindShader(positionShader)
           //node.addChild(positionSprite)



          // var velocityTexture = createTexture(WIDTH,WIDTH);
          // var velocityShader =  createPasThruShader(gl,renderer,passThruUniforms)
          //
          // var velocitySprite = new PIXI.Sprite();
          // velocitySprite.shader = velocityShader;
          // node.addChild(velocitySprite);
          // var attributes = {
          //     aRandom:          { type: '1f' }
          // }
          //
          ...
          //    uPosition:       { type: 'sampler2D' , value: positionTexture},
   ...
   var AMOUNT  = 1000;//window.innerWidth*window.innerHeight/2;//100000
   ...
   const RESOLUTION = 128;//Math.floor(Math.sqrt(AMOUNT));
...
//http://www.vizitsolutions.com/portfolio/webgl/gpgpu/matrixInitialization.html
           ...
           if( ! bufferStatus.isComplete){
            ...
            var GLProgram = new GLCore.GLShader(gl,require('./_defaultVertex.vert'),require('./_defaultProgram.frag'));
              ...
              gl.bindAttribLocation(GLProgram.program, GLProgram.attributes.aPosition.location, "aPosition");
              gl.bindAttribLocation(GLProgram.program, GLProgram.attributes.aTextureCoord.location, "aTextureCoord");
              GLProgram.bind()

              var standardVertices = gl.createBuffer();
             gl.bindBuffer(gl.ARRAY_BUFFER, standardVertices);
             var  getStandardGeometry  = new Float32Array(
             ...
             gl.enableVertexAttribArray(0);
              ...
              gl.vertexAttribPointer(GLProgram.attributes.aPosition.location, 3, gl.FLOAT, gl.FALSE, 20, 0);
              gl.vertexAttribPointer(GLProgram.attributes.aTextureCoord.location, 2, gl.FLOAT, gl.FALSE, 20, 12);
              GLProgram.uniforms.width = RESOLUTION;
              GLProgram.uniforms.height = RESOLUTION;
            ...
            console.log(passed,"TEST: "+ buffer[0] + " at (" + i + ", " + j + "), expected " + expected + ".");
