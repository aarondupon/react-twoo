...
function _initUniforms() {
    ...
    for(id in this.uniforms) {
        ...
        if(uniform.location < 0) {
            ...
            if(uniform.type == 'sampler2D') {//(uniform.type == 't') {//sampler2D
...
_p.render = function(customObject) {
    ...
    var id, ref, buffer, uniform, attribute,bufferType;
    ...
    for(id in uniforms) {
        ...
        if(ref !== undef) {
            if(uniform.isTexture) {
                ...
                if(ref.constructor.name  === 'WebGLTexture'){
              //  if( ref.texture){
...
_p._initWebGL = function(customObject) {
    ...
    for( id in attributes) {
        ...
        gl.bufferData(gl[attribute.bufferType], customObject[id], gl[attribute.usage || 'DYNAM_DRAW']);
    }
};
