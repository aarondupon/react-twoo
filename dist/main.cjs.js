...
function () {
  ...
  _proto[SYMBOL_MATCH] = function () {
    ...
    for (var i = 0; i < arguments.length; i += 1) {
      var tokenDescriptor = i < 0 || arguments$1.length <= i ? undefined : arguments$1[i];
...
var css = unwrapExports(cssToReactNative);
...
var toMillies_1 = createCommonjsModule(function (module, exports) {
...
var _toMillies = _interopRequireDefault(toMillies_1);
...
var getStyle = function getStyle(style) {
  return css(Object.keys(style).map(function (key) {
...
core.utils.mixins.delayMixin(core.DisplayObject.prototype, accessibleTarget$1);
var KEY_CODE_TAB = 9;
...
core.WebGLRenderer.registerPlugin('accessibility', AccessibilityManager); // core.CanvasRenderer.registerPlugin('accessibility', AccessibilityManager);
...
var TextureLoader = function TextureLoader(_uniforms, filter) {
  var _this = this;
...
var BeeldSliderFilter = /*#__PURE__*/function (_PIXI$Filter) {
  _inherits(BeeldSliderFilter, _PIXI$Filter);

  var _super = _createSuper(BeeldSliderFilter);
  ...
  function BeeldSliderFilter() {
    ...
    _defineProperty(_assertThisInitialized(_this), "update", function (t) {
      _this.uniforms.iTime = performance.now() / 1000;
    });
    ...
    var uniforms = {
      ...
      dimensions: {
        type: '4fv',
        value: [0, 0, window.innerWidth, window.innerHeight]
      },
      focus: {
        type: '1f',
        value: 0.1
    ...
    _this.loader = new TextureLoader(textures, uniforms, _assertThisInitialized(_this));
    var filter = PIXI$1.Filter.call(_assertThisInitialized(_this), Vertext, Fragment, uniforms);
    /* end hack */

    _this.animation = animate$1(_this.update, 24);
    _this.animate = true;
    return _this;
  }
  /**
  ...
  _createClass(BeeldSliderFilter, [{
    key: "apply",
    value: function apply(filterManager, input, output, clear) {
      ...
      this.uniforms.iResolution[1] = height; // MOUSE IS NOT NORMALISED !!!!!;

      var mx = this.mouse.x - x;
      var my = this.mouse.y - y;
      var mz = this.mouse.z;
      this.uniforms.iMouse = [0, 0, 0]; // this.uniforms.iMouse = [mx,my,mz]
    ...
    set: function set(texture) {
      ...
      if (!texture || JSON.stringify(texture) == JSON.stringify(this._prev)) { return; } // this.uniforms.prevCol = col[texture.url.match(/\d+/g)[0]];
    ...
    set: function set(texture) {
      console.log('texture:', texture);
      if (!texture || JSON.stringify(texture) == JSON.stringify(this._curr)) { return; } //    this.uniforms.currCol = col[texture.url.match(/\d+/g)[0]];
...
var TextureLoader$1 = function TextureLoader(_textures, uniforms, filter) {
  ...
  _defineProperty(this, "addTextTures", function (textures) {
    textures.forEach(function (file, i) {
      ...
      var texture = sprite.texture;
      var width = texture.width,
          height = texture.height; // add textrue to unifor
      ...
      _this.uniforms["scale_".concat(name)] = {
        type: 'f2',
        value: [ws / sx * ws, hs / sy * hs]
      };
      console.log('addTextTures', "scale_".concat(name), sx, sy, ' | ', ws / sx * ws, hs / sy * hs, '\t\t\t', width, height); // center
      ...
      var tx = -(ws - wn) / 2;
      var ty = (ws - hn) / 2;
      _this.uniforms["transform_".concat(name)] = {
        type: 'f2',
        value: [tx, ty]
      };
      console.log('addTextTures', "transform_".concat(name), '\n ws', ws, '\n wn', wn, '\n tx', tx);
      var mat3 = [sx / ws, 0, 0, 0, sy / hs, 0, tx / ws, ty / hs, 1];
      _this.uniforms["size_matrix_".concat(name)] = {
        type: 'mat3',
        value: mat3
      }; //broll

      _this.uniforms["dimension_".concat(name)] = {
        type: 'f2',
        value: [width, height]
      };
      _this.uniforms["aspect_".concat(name)] = {
        type: 'f1',
        value: height / width
      };
      ...
      if (texture && texture.baseTexture.hasLoaded) ; else {
        texture.baseTexture.wrapMode = PIXI$1.WRAP_MODES.CLAMP;
...
var BeeldHoverFilter = /*#__PURE__*/function (_PIXI$Filter) {
  _inherits(BeeldHoverFilter, _PIXI$Filter);

  var _super = _createSuper(BeeldHoverFilter);
   ...
   * @memberof DisplacementFilter
   */
  function BeeldHoverFilter() {
    var _this2;

    var textures = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    var resolution = arguments.length > 1 ? arguments[1] : undefined;

    _classCallCheck(this, BeeldHoverFilter);
    ...
    console.log('constructor BeeldSlider'); // PIXI.ticker.shared.add(this.update)
  ...
  _createClass(BeeldHoverFilter, [{
    ...
    value: function apply(filterManager, input, output, clear) {
      ...
      var mx = this._mouse.x - x; /// width;//input.size.width//window.innerWidth ;

      var my = this._mouse.y - y; /// height;//input.size.height;

      var mz = this._mouse.z; // map to filter; .6

      this.uniforms.iMouse = [mx, my, mz]; //console.log(this.uniformData)
      //  console.log('mx',mx)
...
var BeeldSliderFilter$1 = /*#__PURE__*/function (_PIXI$Filter) {
  _inherits(BeeldSliderFilter, _PIXI$Filter);
  ...
  function BeeldSliderFilter() {
    ...
    _this.animation = animate(_this.update, 24);
    _this.animate = true;
    return _this;
  }
  /**
...
exports.BeeldSliderFiler = BeeldSliderFilter;
exports.BeeldSliderFiler_1 = BeeldHoverFilter;
exports.BeeldSliderFiler_2 = BeeldSliderFilter$1;
